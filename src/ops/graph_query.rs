...
impl OpHandler for GraphQueryHandler {
    fn name(&self) -> &'static str { "graph.query" }
    fn description(&self) -> &'static str { "Query the page graph to a given depth" }
    ...
    fn from_mcp_args(&self, args: Option<serde_json::Map<String, Value>>) -> Result<Box<dyn OpExec>, anyhow::Error> {
        ...
        let depth = args.get("depth")
            .and_then(|v| v.as_u64())
            .map(|d| d as usize);
        let link_type = args.get("link_type")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        let direction = args.get("direction")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        Ok(Box::new(GraphQueryOp { slug, depth, link_type, direction }))
    }
    ...
    fn cli_command(&self) -> clap::Command {
        clap::Command::new("query")
            ...
            .arg(clap::Arg::new("depth").long("depth").value_parser(clap::value_parser!(usize)))
            .arg(clap::Arg::new("link_type").long("link-type"))
            .arg(clap::Arg::new("direction").long("direction").value_parser(["in", "out", "both"]))
    }
...
mod tests {
    ...
    fn test_graph_query_in_inventory() {
        ...
        assert!(names.contains(&"graph.query"), "graph.query should be in inventory, found: {:?}", names);
    }
    ...
    fn test_graph_query_from_mcp_args_with_filters() {
        ...
        args.insert("slug".to_string(), serde_json::Value::String("test/page".to_string()));
        args.insert("depth".to_string(), serde_json::Value::Number(2.into()));
        args.insert("link_type".to_string(), serde_json::Value::String("cites".to_string()));
        args.insert("direction".to_string(), serde_json::Value::String("both".to_string()));
