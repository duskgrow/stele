...
pub struct PageDeleteHandler;

/// Executable operation with parsed arguments.
pub struct PageDeleteOp {
    pub slug: String,
}

#[async_trait]
impl OpExec for PageDeleteOp {
    fn as_any(&self) -> &dyn std::any::Any {
        self
    }
    async fn execute(&self, ctx: &OperationContext) -> Result<Value, anyhow::Error> {
        crate::ops::page::handle_page_delete(&ctx.fns, &ctx.index, &self.slug)
            .await
            .map_err(|e| anyhow::anyhow!("{e}"))
    }
}

impl OpHandler for PageDeleteHandler {
    fn name(&self) -> &'static str { "page.delete" }
    fn description(&self) -> &'static str { "Delete a page by slug" }
...
inventory::submit! {
    &PageDeleteHandler as &'static dyn OpHandler
}
...
mod tests {
    ...
    #[test]
    fn test_page_delete_handler_meta() {
        let handler = PageDeleteHandler;
        assert_eq!(handler.name(), "page.delete");
    ...
    #[test]
    fn test_page_delete_from_mcp_args() {
        let handler = PageDeleteHandler;
    ...
    #[test]
    fn test_page_delete_missing_slug() {
        let handler = PageDeleteHandler;
    ...
    #[test]
    fn test_page_delete_in_inventory() {
        let handlers: Vec<_> = inventory::iter::<&'static dyn OpHandler>
            .into_iter()
            .collect();
        let names: Vec<&str> = handlers.iter().map(|h| h.name()).collect();
        assert!(names.contains(&"page.delete"), "page.delete should be in inventory, found: {:?}", names);
    }
}
