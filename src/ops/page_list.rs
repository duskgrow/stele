...
impl OpHandler for PageListHandler {
    fn name(&self) -> &'static str { "page.list" }
    fn description(&self) -> &'static str { "List pages" }
...
mod tests {
    ...
    fn test_page_list_in_inventory() {
        ...
        assert!(names.contains(&"page.list"), "page.list should be in inventory, found: {:?}", names);
    }
}
